<?php
   ...
   if(isset($_POST["login_remember"])) {
      ...
      if (isset($_POST["username"])) {
         setcookie ("login_remember", protect($_POST["username"]), time()+ (10 * 365 * 24 * 60 * 60),'/');
      }
   ...
   if (isset($_POST["username"]) && isset($_POST["password"]))
   {
      ...
      $sql = "SELECT * FROM agents WHERE username='".$username_escaped."' AND password='".$password_escaped."' LIMIT 1";
      ...
      if ($result && mysqli_num_rows($result)==1)
      {
         ...
         $update_sql = "UPDATE agents SET last_log = '".date("Y-m-d H:i:s")."' WHERE username='".$username_escaped."' AND password='".$password_escaped."' LIMIT 1";
