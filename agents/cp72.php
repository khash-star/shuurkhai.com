<?php 
// Start output buffering to catch any errors before image output
ob_start();
...
// Use __DIR__ to get absolute paths
$base_dir = __DIR__;
$ds = DIRECTORY_SEPARATOR;
$cp72Path = $base_dir . $ds . 'assets' . $ds . 'img' . $ds . 'cp72.jpg';
$logo_path = $base_dir . $ds . 'assets' . $ds . 'img' . $ds . 'logo.jpg';
$font_path = $base_dir . $ds . 'assets' . $ds . 'fonts' . $ds . 'Arial.ttf';
...
// All checks passed, now we'll set header after generating barcode
// Don't set header yet - wait until we're ready to output image
...
if (mysqli_num_rows($result)==1)
{
    $data = mysqli_fetch_array($result);
        ...
        $is_branch = isset($data["is_branch"]) ? $data["is_branch"] : 0;
   		...
   		//SENDER 
			...
			$sender_first_name = !empty($sender_name) ? explode(' ', $sender_name)[0] : '';
        ...
        try {
            ...
            // If barcode generation fails, continue without barcode
            error_log("Barcode generation failed: " . $e->getMessage());
        }
        $barcode_temp_path = $base_dir . $ds . 'assets' . $ds . 'img' . $ds . 'barcode_temp.jpg';
    
        ini_set('memory_limit', '-1');
        $font = $font_path;
        $font_size = 15;
        
        // Verify font file exists and is readable BEFORE setting header
        if (!file_exists($font) || !is_readable($font)) {
            if (ob_get_level() > 0) ob_clean();
            header('Content-Type: text/html; charset=utf-8');
            die("Алдаа: Фонт файл уншиж чадсангүй: " . $font);
        }
        
        // Create image - errors should be caught before header is set
        $image = @imagecreatefromjpeg($cp72Path);
        if (!$image) {
            $error_msg = "Cannot load CP72 template image. Path: " . $cp72Path;
            error_log($error_msg);
            if (ob_get_level() > 0) ob_clean();
            header('Content-Type: text/html; charset=utf-8');
            die("Алдаа: " . $error_msg);
        ...
        // Now that everything is ready, set image header and clear output buffer
        if (ob_get_level() > 0) {
            ob_clean();
        }
        ...
        if ($is_branch)          
        imagettftext($image,$font_size,0,800,120,$color,$font,number_format(cfg_price_branch($weight), 2, '.', ',')."$");
        else 
        ...
        */
        ...
        // Copy barcode and logo with error suppression
        ...
        // Output image
        imagejpeg($image, null, 90); // Use quality 90
        imagedestroy($image);
        if (isset($barcode_image_source)) imagedestroy($barcode_image_source);
        if (isset($logo_image_source)) imagedestroy($logo_image_source);
