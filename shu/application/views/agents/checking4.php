...
<? 
	...
	if ($order_id!="")
	{
			...
			if ($current_proxy==0)
				{
						...
						$query_proxies = $this->db->query('SELECT * FROM proxies WHERE customer_id="'.$customer_id.'" ORDER BY proxy_id DESC');
						$proxy_available_id=0;
						if ($query_proxies->num_rows()>1)
						{
							foreach($query_proxies->result() as $row_proxy)
							{
								$proxy_id_instance = $row_proxy->proxy_id;
								$order_proxy = $this->db->query('SELECT proxy_id FROM orders WHERE receiver="'.$customer_id.'" AND created_date>="'. date('Y-m-d', strtotime(date('Y-m-d') . ' -1 month')).'" AND proxy_id="'.$proxy_id_instance.'" AND status NOT IN ("delivered","warehouse","custom","onair")');	
								$combine_proxy = $this->db->query('SELECT proxy_id FROM box_combine WHERE receiver="'.$customer_id.'" AND created_date>="'. date('Y-m-d', strtotime(date('Y-m-d') . ' -1 month')).'" AND proxy_id="'.$proxy_id_instance.'" AND status NOT IN ("delivered","warehouse","custom","onair")');
								
								if ($order_proxy->num_rows() == 0 && $combine_proxy->num_rows()==0) 
									{$proxy_available_id= $proxy_id_instance; }
							}
						}
						...
						{
							//echo "BEFORE PUBLIc PROXY: ".$proxy_id."<br>----------<br>";
		...
		$order_proxy = $this->db->query('SELECT * FROM orders WHERE receiver="'.$customer_id.'" AND proxy_id=0 AND status NOT IN ("delivered","warehouse","custom","onair")');
		//echo 'TESTING'.'SELECT * FROM orders WHERE receiver="'.$customer_id.'" AND proxy_id=0 AND status NOT IN ("delivered","warehouse","custom","onair")';
