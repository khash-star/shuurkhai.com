...
if ($query->num_rows() == 1)
{
   $row = $query->row();
		...
		$proxy_contact=proxy2($proxy,$proxy_type,"tel2cp");
	...
	if ($receiver==6616) 
	$cp72Path ='assets/images/cp72_kr.jpg';
	//if (substr($barcode,0,2)!="GO" && )
	...
	if ($receiver==6616) 
	$cp72Path ='assets/images/cp72_kr.jpg';
	//if (substr($barcode,0,2)=="KR")
	//$logo_image_source = imagecreatefromjpeg('assets/images/logo_kr.jpg');
	...
	if ($receiver!=6616) 
	{
		...
		if ($is_branch) imagettftext($image,$font_size,0,20,195,$color,$font,"1   9  8   0  6");


		// imagettftext($image,$font_size,0,40,195,$color,$font,"9");
		// imagettftext($image,$font_size,0,25,195,$color,$font,"8");
		// imagettftext($image,$font_size,0,25,195,$color,$font,"0");
		// imagettftext($image,$font_size,0,25,195,$color,$font,"6");
	...
	if ($receiver==6616) 
	 {
	 	imagettftext($image,$font_size+8,0,90,95,$color,$font,"Kargo.mn");
		imagettftext($image,$font_size,0,255,195,$color,$font,$sender_contact);
		imagettftext($image,$font_size-3,0,90,120,$color,$font,"4712 OAKTON STREET");
		imagettftext($image,$font_size-3,0,90,140,$color,$font,"Skokie IL");
		//imagettftext($image,$font_size,0,79,160,$color,$font,substr($sender_address,52,18));
	 }
	...
	//if (substr($weight,-2,2)=="kg") $weight=substr($weight,0,strlen($weight)-2);
	if ($is_branch) $weight_price = cfg_price_branch($weight); else $weight_price = cfg_price($weight);

	imagettftext($image,$font_size,0,800,120,$color,$font,$weight_price."$");
