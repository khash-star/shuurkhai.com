				...
				if (substr($contact,0,1)=="K" || substr($contact,0,1)=="К")
					{
						...
						$customer_id=6616;
						$query_proxy = $this->db->query("SELECT * FROM proxies WHERE customer_id =6616 AND  code='$contact'");
						$row = $query->row();
						...
						echo form_open('agents/tracks_checking4');
							...
							echo "<p>Карго.мн-р захиалсан ачаа байна.</p>";
					...
					if ($query_receiver->num_rows() == 1)
					 {
							...
							echo '<h3>'.$surname.' овогтой '.$name.' '.anchor("agents/tracks_clean/".$order_id,"Биш ахин оруулах",array('class'=>'btn btn-warning pull-right')).'</h3>';
							echo form_open('agents/tracks_checking4');
							echo form_hidden("track",$track);
...
if ($order_id=="")
{
	...
	if ($container_id!="") 
		{
			$query_container_item = $this->db->query("SELECT * FROM container_item WHERE id =$container_id LIMIT 1");
			$row = $query_container_item->row();
			$created_date=$row->created_date;
			$barcode=$row->barcode;
			$track=$row->track;
			$sender=$row->sender;
			...
			echo "<tr><td>Нийт жин эсвэл хэмжээ /кг/(*)</td><td>".form_input ("weight","",array("class"=>"form-control","id"=>"weight","required"=>"required"))."</td></tr>";
