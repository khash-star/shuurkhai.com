...
<? 
	...
	if ($no_proxy==0)
	{
		if (isset($_POST["proxy"]) && $current_proxy==0)
			{
				$proxy =$_POST["proxy"];
				if ($proxy=='yes')
				{
					...
					if ($query_proxies->num_rows()>1)
					{
						foreach($query_proxies->result() as $row_proxy)
						{
							...
							$order_proxy = $this->db->query('SELECT proxy_id FROM orders WHERE receiver="'.$customer_id.'" AND proxy_id="'.$proxy_id_instance.'" AND status NOT IN ("delivered","warehouse","custom","onair")');	
							$combine_proxy = $this->db->query('SELECT proxy_id FROM box_combine WHERE receiver="'.$customer_id.'" AND proxy_id="'.$proxy_id_instance.'" AND status NOT IN ("delivered","warehouse","custom","onair")');
	...
	//echo "FINAL PROXY:".$proxy_id."Before ITEM MISSING<br>---------------<br>";
	/*if ($current_status=="item_missing")
		{
			$query_current = $this->db->query('SELECT order_id FROM orders WHERE receiver="'.$customer_id.'" AND proxy_id=0 AND status="item_missing"');
		if ($query_current->num_rows()==1)
			{
			 	$proxy_id=0;
				$proxy_type =0;// 0-own proxy, 1-public proxy
			}

		}*/
	//echo "FINAL PROXY:".$proxy_id."after ITEM MISSING<br>---------------<br>";
	$query = $this->db->query("SELECT * FROM orders WHERE SUBSTRING(third_party,-8,8) = '$track_eliminated' LIMIT 1");
	if ($query->num_rows() == 1)  // тухай barcode-н утасны дугаар оруулж хэрэглэгч хайх
		{
		$row = $query->row();
