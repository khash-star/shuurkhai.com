<? if ($this->uri->segment(3)) $container_item_id=$this->uri->segment(3); else exit();
//ORDER STATUS CHANGE INTO new
$query_order = $this->db->query('SELECT status FROM container_item WHERE id="'.$container_item_id.'"');
if ($query_order->num_rows()==1)
{
	$row = $query_order->row();
	if ($row->status=="filled")  
	$this->db->query('UPDATE container_item SET status="new" WHERE id="'.$container_item_id.'"');
...
$query = $this->db->query("SELECT * FROM container_item WHERE id=".$container_item_id);
...
if ($query->num_rows() == 1)
    {
        ...
        $payment= intval($row->payment);
        $pay_in_mongolia= intval($row->pay_in_mongolia);
	...
	$font_size=15;
    $cp72Path ='assets/images/cp72_container_ex.jpg';
