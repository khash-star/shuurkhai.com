<? if (!$this->uri->segment(3)) redirect('admin/proxy'); else $proxy_id=$this->uri->segment(3); ?>
...
<? 
$query = $this->db->query("SELECT * FROM proxies_public WHERE proxy_id=".$proxy_id);
...
if ($query->num_rows() == 1)
{
	...
	echo "</table>";
		...
		if ($this->db->query("DELETE FROM proxies_public WHERE proxy_id=".$proxy_id)) 
