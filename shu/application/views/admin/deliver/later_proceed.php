...
if ($query->num_rows() > 0)
	   ...
	   echo "<th>Төлбөр</th>"; 
	   echo "</tr>";
	   $count=1;$total_weight=0;$total_payment=0;
	   $data = array(array('№','Х/авагч','Утас','Barcode','Ширхэг','Жин','Төлбөр'));
		...
		if ($total_payment>0)
		   ...
		   echo "</tr>";
		   $grand_weight +=$grand_total +=$total_payment;
		   $grand_total +=$total_payment;
		    array_push($data,array($count,$customer_name,$customer_tel,implode(", ",$barcodes_array),count($barcodes_array),$grand_weight,$total_payment));	
			$count++;
			}
		$method="";
