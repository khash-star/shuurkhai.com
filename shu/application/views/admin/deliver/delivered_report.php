...
if ($query->num_rows() > 0)
	   ...
	   $data = array(array('№','Х/авагч','Утас','Barcode','Ширхэг','Жин','Төлбөр','Төрөл'));
		foreach ($query->result() as $row)
			...
			if ($method_type=="all")   $sql.=" AND orders.method <>'later'";
			if ($method_type!="all") $sql.=" AND orders.method ='$method_type' AND orders.method <>'later'";
			if ($start_date!="")  $sql.=" AND delivered_date>'$start_date'";
			if ($finish_date!="")  $sql.=" AND delivered_date<'$finish_date'";
		...
		//	echo "<br>".$sql;
			...
			foreach ($query_detail->result() as $row_detail)
				...
				if ($row_detail->is_online == 0)
					{
					$advance+=floatval($row_detail->advance_value);
	 ...
	 echo "<tr><td colspan='6'>Нийт</td><td>$grand_total</td><td></td></tr>";
	 echo "</table>";
