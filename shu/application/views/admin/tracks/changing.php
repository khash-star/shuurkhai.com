...
<? 
echo "<h3>Online илгээмж өөрчлөлт</h3>";
...
foreach ($query->result() as $row)
	   ...
	   $days= (strtotime(date("Y-m-d")) - strtotime(date("Y-m-d",strtotime($created_date))))/3600/24;
		...
		if($new_status=="delivered") 
		{
			if ($deliver_id!=1 && $status=="warehouse")
			$data = array(
               'status' => $new_status,
			   'deliver' => $deliver_id
            		  );	
		...
		if($new_status=="onair")
		{
		if ($status!="onair" && $status!="warehouse" && $status!="delivered" && $status!="custom")
		$data = array(
               'status' => $new_status,
			 //  'extra' => $extra,
			   'onair_date' => date("c")
            		  );	
		else $error = "onair,warehouse,delivered эсвэл custom төлөвт байна";
		}
	...
	else echo "Online илгээмж тэмдэглэгдээгүй байна";
?>
