...
if ($query->num_rows() > 0)
{
	...
	echo "<td>";
		...
		echo '<span style="color:#0F0; font-weight:bold;">Дүн бодогдоогүй: '.$row->total_new.'ш</span><br>';
		...
		echo '<span style="color:#f0861b; font-weight:bold;">Shipping: '.number_format($row->total_shipping,2).'$</span><br>';
	 echo '<span style="color:#ff0000; font-weight:bold;">Дараа төлбөр: '.number_format($row->total_owe,2).'$</span><br>';
			$total = $row->total_price+$row->total_tax+$row->total_shipping+$row->total_owe;
	echo '<span style="color:#be0fce; font-weight:bold;font-size:14px;">Нийт: '.number_format($total,2).'$ ('.number_format($total*cfg_rate()).'₮)</span><br>';
	//echo '<span style="color:#be0fce; font-weight:bold;font-size:14px;">Нийт: '.floor($total*cfg_rate()).'₮</span>';
	...
	if ($query_single->num_rows() > 0)
	{
		...
		echo "<tr><th>№</th><th>Үүсгэсэн</th><th>Үнэ</th><th>Коммент</th><th>Тайлбар</th><th width='130'>Үйлдэл</th></tr>";
		...
		echo '<span style="color:#f0861b; font-weight:bold;">Shipping: '.$shipping.'$</span><br>';
		echo '<span style="color:#ff0000; font-weight:bold;">Дараа төлбөр: '.$owe.'$</span>';
		echo "<td>".$comment."</td>";	
		echo "<td>".$context."</td>";
		...
		echo "<td>";
			...
			echo"<br>";
			echo anchor('admin/online_pending/'.$online_id,'<span class="glyphicon glyphicon-refresh"></span> Track pending',array('title'=>"Track pending!","class"=>"btn btn-xs btn-success btn-shuurkhai"));
			echo"<br>";
			...
			echo "<br>";
			echo anchor('admin/online_later/'.$online_id,'<span class="glyphicon glyphicon-transfer"></span> Дараа нь',array('title'=>"Make later!","class"=>"btn btn-xs btn-default btn-shuurkhai"));
			echo "<br>";
