<? 
if ($this->uri->segment(3)) $combine_id=$this->uri->segment(3);
...
$query = $this->db->query("SELECT * FROM box_combine WHERE combine_id='".$combine_id."'");
if ($query->num_rows() == 1)
{
   $row = $query->row();
	...
	imagettftext($image,$font_size,0,800,120,$color,$font,$weight*cfg_paymentrate()."$");
	...
	imagecopymerge($image, $barcode_image_source, 550, 530, 0,0,334, 69,100);
