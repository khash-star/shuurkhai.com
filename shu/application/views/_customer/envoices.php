...
$sql = "SELECT * FROM envoice WHERE customer_id=".$customer_id." ORDER BY created_date DESC";
...
if ($query->num_rows() > 0)
{
		...
		$orders_array=explode(",",$row->orders);
		$created_date=$row->created_date;
		$amount=$row->amount;
		
		echo "<tr>";
		echo "<td>".$i++."</td>";
		echo "<td width='90'>".substr($created_date,0,10)."</td>";
		echo "<td>".$amount."₮</td>";
		echo "<td>";
		...
		echo "<td>";
		echo anchor("customer/envoices_display/".$envoice_id,"Харах",array("class"=>"btn btn-xs btn-success","target"=>"new"));
