...
if(isset($_POST['orders'])) 
{
	...
	if ($this->db->insert('envoice', $data));
	$envoice_id = $this->db->insert_id();
	}
	else 
	{
		$row = $query->row();
...
<div class="panel">
<img src="<?=base_url("assets/images/logo.png");?>" style="margin:auto; height:40px; float:left; " />
<div class="clearfix"></div>
  <div class="panel-heading" style="text-align:center; font-weight:bold;">Нэхэмжлэх №<?=sprintf("%07d", $envoice_id);?></div>
...
<table width="100%">
  <tr><td align="left" width="50%">Нэхэмжлэгч:<br />Shuurkhai Cargo </td>
    ...
    <tr><td align="left" width="50%">Хаяг:<br /> 4712 OAKTON STREET, Chicago, IL, 60076</td>
    ...
    <tr><td align="left">  Email:<br />info@shuurkhai.com </td><td> Email:<br /><?=customer($customer_id,"email");?></td></tr>
     ...
     <tr><td align="left">
     Долларын данс<br />
     Банкны нэр: ХХБанк<br />
    Банкны дансны нэр: Хашбал<br />
    Доллар:416002824	<br />	 
    
    Төгрөгийн данс данс<br />
    Банкны нэр: Хаан банк<br />
    Банкны дансны нэр: Хашбал 		<br />
    Төгрөг:5753899300	<br /></td><td></td></tr>
    </table>
...
<?
	...
	echo "<tr><td colspan='3' align='right'>Төлбөр($)</td><td>".$total_weight*cfg_paymentrate()."</td></tr>";
	echo "<tr><td colspan='3' align='right'>НӨАТ</td><td>0</td></tr>";
	echo "<tr><td colspan='3' align='right'>Нийт төлбөр($)</td><td>".$total_weight*cfg_paymentrate()."</td></tr>";
	$amount=$total_weight*cfg_paymentrate()*cfg_rate();
	echo "<tr><td colspan='3' align='right'>Нийт төлбөр(₮)</td><td>".$amount."</td></tr>";
	echo "</table>";
	...
	$this->db->where('envoice_id', $envoice_id);
	$this->db->update('envoice', $data);
