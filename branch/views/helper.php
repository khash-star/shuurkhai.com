...
if (! function_exists ('tracksearch'))
{
	function tracksearch($track)
	{
		...
		if (substr($track,0,2)=='22' || substr($track,0,2)=='23' || substr($track,0,2)=='ES')
			$result = mysqli_query($conn,"SELECT * FROM orders WHERE third_party = '$track' AND created_date>='$filter_date 00:00:00' LIMIT 1");
			$result=mysqli_query($conn,"SELECT * FROM orders WHERE third_party = '$track' AND created_date>='$filter_date 00:00:00' LIMIT 1");
		if (substr($track,0,2)!='22' && substr($track,0,2)!='23' && substr($track,0,2)!='ES')
			{
				$track_eliminated = substr($track,-8,8);
				$result = mysqli_query($conn,"SELECT * FROM orders WHERE SUBSTRING(third_party,-8,8) = '$track_eliminated' AND created_date>='$filter_date 00:00:00' LIMIT 1");
