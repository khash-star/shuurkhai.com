$(document).ready(function(){
    ...
    $("#tel").inputmask({mask:"99999999"});
    //$("#name_crillic").inputmask({mask:"[A]{2,20}"});
    $("#name_crillic").inputmask({
    mask:"*{1,20}",
    greedy:!1,onBeforePaste:function(m,a){return(m=m.toLowerCase()).replace("mailto:","")},
    definitions:{"*":
        {
            validator:"[ФЦУЖЭНГШҮЗКЪЕЩЙЫБӨАХРОЛДПЯЧЁСМИТЬВЮфцужэнгшүзкъещйыбөахролдпячёсмитьвю-]"        
        }
    }});
    $("#surname_crillic").inputmask({
        mask:"*{1,20}",
        greedy:!1,onBeforePaste:function(m,a){return(m=m.toLowerCase()).replace("mailto:","")},
        definitions:{"*":
            {
